...
!function(window, $, undefined){
    ...
    jBar.prototype = {
        init: function () {
            ...
            this.construct().printMessage().removeData().togglejBar()
        },
        construct: function () {
            this.$elem.before(
                $('<div class="jbar-push"></div>' +
                    '<a href="#" class="jbar-down-toggle">' +
                    '<span class="jbar-no">('+this.jsonConfig.errNo+'!)</span>'+
                    //'<img src="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7" class="jbar-down-arrow" alt="">'+
                    '</a>')
                )
            this.$elem.append(
                $(			
                    '<div class="jbar-wrap"></div>'+
                    '<p class="jbar-buttons"><a href="' + this.jsonConfig.url + '" class="jbar-button">' + this.jsonConfig.button + '</a>'+
                    '<a href="#" class="jbar-up-toggle jbar-up-arrow">'
                    )
                )
            return this
        },
